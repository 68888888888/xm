import React, { Component } from 'react';
import { Switch, Route, NavLink, Redirect, withRouter } from 'react-router-dom';
import '../main.scss';
import home from '../pages/home';
import buy from '../pages/buy';
import user from '../pages/user';
import find from '../pages/find';
import shop from '../pages/shop';
import AuthRouter from '../AuthRouter'
class App extends Component {
  // constructor(props){
  //   super(props)
  //     this.state={
  //        url:this.props.location.pathname.match(/\/(\S*)\//)[1]
  //     }
      
  // }
  gohome(){
    console.log(this)
     this.props.history.push('/home')
  }
  
  render() {
    return (
      <div className="App">
         <header>
         
           <div className="header_war">
             <p className="logo" onClick={this.gohome.bind(this)}><img src="data:image/png;base64," alt=""/></p>
                <NavLink to = "/home" className={this.props.location.pathname==='/home'?'action':''}>首页</NavLink>
                <NavLink to = "/buy/365" className={this.props.location.pathname==='/buy/365'?'action':''}>购票</NavLink>
                <NavLink to = "/shop" className={this.props.location.pathname==='/shop'?'action':''}>商城</NavLink>
                <NavLink to = "/find" className={this.props.location.pathname==='/find'?'action':''}>发现</NavLink>
                <NavLink to = "/user" className={this.props.location.pathname==='/user'?'action fa fa-user-o':'fa fa-user-o'}></NavLink> 
                
              </div>  
         </header>
         <main>
            <Switch>
              <Route path='/home' component={home}></Route>
              <Route path='/buy/:hehe' component={buy}></Route>
              <AuthRouter path='/user' component={user}></AuthRouter>
              <Route path='/find' component={find}></Route>
              <Route path='/shop' component={shop}></Route>  
              <Route path='/' exact render={()=> (
              <Redirect to='/home'/>
           )}/>
            </Switch>
         </main>
         <footer>
           <div className="nav">
                <NavLink to = "/home" >首页</NavLink>
                <NavLink to = "/buy" >购票</NavLink>
                <NavLink to = "/shop" >商城</NavLink>
                <NavLink to = "/find" >发现</NavLink>
                <NavLink to = "/user" >我的</NavLink> 
           </div>
           <div className="bottom">
             <ul>
               <li>pc版</li>
               <li>客户端下载</li>
               <li>意见反馈</li>
               <li>帮助中心</li>
             </ul>
             <div className='tu'>
                 <p><img src="https://static1.mtime.cn/html5/20190523161337/images/2014/fot_logo.png" alt=""/></p>
                 <p><img src="https://static1.mtime.cn/html5/20190523161337/images/h_mark.jpg" alt=""/></p>
             </div>
             <p className='zi'>Copyright 2006-2019  Mtime.com Inc. All rights reserved.</p>
           </div>
         </footer>
      </div>
    );
  }
}

export default withRouter(App);
